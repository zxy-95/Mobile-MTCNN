...
class MtcnnDetector(object):
    ...
    def detect_rnet(self, im, dets):
        ...
        for i in range(num_boxes):
            ...
            cropped_ims[i, :, :] = (cv2.resize(tmp, (24, 24),interpolation=cv2.INTER_NEAREST)-127.5) / 128
        cropped_ims=np.reshape(cropped_ims,(num_boxes,24,24,1))
        print(num_boxes)
        ...
        # boxes = boxes[py_nms(boxes, 0.3, "Minimum")]
        # keep = py_nms(boxes_c, 0.3, "Minimum")
    ...
    def detect_onet(self, im, dets):
        ...
        [dy, edy, dx, edx, y, ey, x, ex, tmpw, tmph] = self.pad(dets, w, h)
        num_boxes = dets.shape[0]
        cropped_ims = np.zeros((num_boxes, 48, 48), dtype=np.float32)
        for i in range(num_boxes):
            tmp = np.zeros((tmph[i], tmpw[i]), dtype=np.uint8)
            tmp[dy[i]:edy[i] + 1, dx[i]:edx[i] + 1] = im[y[i]:ey[i] + 1, x[i]:ex[i] + 1]
            cropped_ims[i, :, :] = (cv2.resize(tmp, (48, 48),interpolation=cv2.INTER_NEAREST)-127.5) / 128
        cropped_ims = np.reshape(cropped_ims, (num_boxes, 48, 48, 1))
        print(num_boxes)
