...
def test(stage, testFolder):
    ...
    for idx, imagePath in enumerate(testImages):
        ...
        for bbox in allBoxes[idx]:
            ...
            total_face += 1
            print(total_face)
            # cv2.imshow("no",cropped)
            # cv2.waitKey(0)
            cv2.putText(image, str(np.round(bbox[4], 2)), (int(bbox[0]), int(bbox[1])), cv2.FONT_HERSHEY_TRIPLEX, 0.5,
                        color=(255, 0, 255))
            cv2.rectangle(image, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (0, 0, 255), thickness=1)
...
if __name__ == "__main__":
    ...
    test(stage, 'E:\database\headtracker_sequences\seq_mb')
